...
from loss import ArcFace, Circle
...
class IBNNet(nn.Module):
    def __init__(self, num_classes=3, num_aux=23, feat_dim=512, metric_loss='ArcFace', dropout=0., m=0.5, depth=50, pretrained=True, path='../input/backbone_pretrained/resnet101_ibn_a.pth.tar'):
        super().__init__()
        ...
        if self.metric_loss == 'ArcFace':
            ...
            raise ValueError('Please select one from ["ArcFace", "Circle", "Softmax"]')
    ...
    def forward(self, x, label=None): # label is only used when using cos_layer
        ...
        if self.training:
            if self.metric_loss == 'Softmax':
                cls_score = self.classifier(feat)
            else:
                cls_score = self.metric(feat, label)
...
class SNet(nn.Module):
    def __init__(self, num_classes=3, num_aux=23, feat_dim=512, metric_loss='ArcFace', dropout=0., m=0.5, depth=50, pretrained=True):
        super().__init__()

        self.feat_dim = feat_dim
        self.metric_loss = metric_loss

        self.base = build_resnest_backbone(depth, pretrained)
